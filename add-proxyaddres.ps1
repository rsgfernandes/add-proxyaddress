...
Foreach ($user in $users) {
                ...
                Else {
				Get-ADUser -Filter "SamAccountName -eq '$($user.samaccountname)'" -Properties * | Set-ADUser -Add @{Proxyaddresses="SMTP:"+$user.EmailAddress.split('@')[0]+""+$defaultemailaddress}
				Get-ADUser -Filter "SamAccountName -eq '$($user.samaccountname)'" -Properties * | Set-ADUser -Add @{Proxyaddresses="smtp:"+$user.EmailAddress.split('@')[0]+""+$newproxydomain}
				}
}